...
<h3>{{$exam->exam_name}}<span class="pull-right"><button class="btn btn-xs btn-success" role="button" id="btnPrint{{$exam->id}}"><i class="material-icons">print</i> @lang('Результат принта')</button></span></h3>
<div class="visible-print-block" id="table-content{{$exam->id}}">
  <table class="table table-bordered" style="font-size: 10px;">
    <thead>
      <tr>
        <th scope="col">#</th>
        <th scope="col">@lang('Курс')</th>
...
<div class="table-responsive">
  ...
  <tbody>
    ...
    <tr id="heading{{($loop->index + 1)}}">
      ...
      <td>{{$grade->course->course_name}}</td>
      <td><b>{{$grade->marks}}</b>
        <a class="btn btn-xs btn-danger pull-right" href="#collapse{{($loop->index + 1)}}" role="button" data-toggle="collapse" aria-expanded="false" aria-controls="collapse{{($loop->index + 1)}}"> @lang('Посмотреть инфо.')</a>
      </td>
      <td>
        @foreach($gradesystems as $gs)
         @if($grade->marks >= $gs->from_mark && $grade->marks <= $gs->to_mark)
