<div id="my_upload">
    @if($upload_type != 'profile')
        @if((ucfirst($upload_type) == "Syllabus"))
        <h3>Силлабус</h3>
        @else
        <h3>Событие</h3>
        @endif
        <br>
        <label for="upload-title">@lang('Название файла') : </label>
        <input type="text" class="form-control" name="upload-title" id="upload-title" placeholder="@lang('Название файла здесь ...')" required>
        <br/>
    @endif
    ...
    @endif<br>
  <input class="form-control-sm btn btn-warning" id="fileupload" type="file"  accept=".xlsx,.xls,.doc,.docx,.ppt,.pptx,.txt,.pdf,image/png,image/jpeg" name="file" data-url="{{url('upload/file')}}">
  <br/>
  <div class="progress">
    <div class="progress-bar progress-bar-striped active" id="up-prog-info" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%">
      <div class="text-xs-center" id="up-prog-info">0% @lang('Загружено')</div>
    </div>
  </div>
...
$(function () {
    ...
    var uploadButton = $('<button/>')
            ...
            .text(@json( __('Загрузить')))
            .on('click', function () {
                @if($upload_type != 'profile')
                    if(!$('#upload-title').val()){
                        swal({
                            title:@json( __('Напишите название файла')),
                            type:'info',
                ...
                $('#fileupload').hide();
                    ...
                    if(file['type'].length && !acceptFileTypes.test(file['type'])) {
                        $('#fileupload').show();
                        swal(@json( __('Неверный тип файла')));
                        $this.remove();
                        return false;
                    } else if(file.size > filesSize) {
                        swal(@json( __('Размер файла слишком большой \n Не должен превышать : ')) + filesSize + 'MB');
                        $this.remove();
                        return false;
                    }else {
                        $('#up-prog-info').text(@json( __('Загружается...')));
                        $this.off('click').text(@json( __('Прервать'))).on('click', function () {
                            $this.remove();
                            data.abort();
    ...
    $('#fileupload').fileupload({
        ...
        progress: function (e, data) {
            ...
            console.log('progress'+progress);
                ...
                $('#up-prog-info').text(progress + "% " + @json( __('загруженно')));
        }
    })
