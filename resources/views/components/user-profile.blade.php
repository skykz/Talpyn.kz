<div class="row">
  ...
  <div class="col-md-10" id="main-container">
    <h3>{{$user->name}} <span  class="label label-warning">{{ucfirst($user->role)}}</span>
      @if ($user->role == 'teacher' && $user->section_id > 0)
        <small> @lang(' Преподаватель класса раздела'): <span class="label label-info">{{ucfirst($user->section->section_number)}}</span></small>
      @endif
      
      @if($user->role == "student")
       <button class="btn btn-xs btn-success pull-right" role="button" id="btnPrint"><i class="material-icons">print</i> @lang('Распечатать Профиль')</button>
       ...
       <div class="row">
          <div class="col-md-12">
            ...
            <div class="col-xs-4">
              ...
              <div style="font-size: 10px;">@lang('Распечатать даты'): {{Carbon\Carbon::now()->format('d/m/Y')}}</div>
            </div>
          </div>
        </div>
        <br/>
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="col-xs-3">
              @if(!empty($user->pic_path))
              <img src="{{asset('01-progress.gif')}}" data-src="{{url($user->pic_path)}}" class="img-thumbnail" id="my-profile" alt="@lang('Аватра профиля')" width="120px" height="120px">
              @else
              @if(strtolower($user->gender) == trans('male'))
                <img src="{{asset('01-progress.gif')}}" data-src="https://png.icons8.com/dusk/200/000000/user.png" class="img-thumbnail" id="my-profile" alt="@lang('Аватра профиля')" width="120px" height="120px">
              @else
                <img src="{{asset('01-progress.gif')}}" data-src="https://png.icons8.com/dusk/200/000000/user-female.png" class="img-thumbnail" id="my-profile" alt="@lang('Аватра профиля')" width="120px" height="120px">
              @endif
              @endif
            </div>
          </div>
    ...
    <table class="table table-bordered table-striped">
      <tbody>
        <tr>
          ...
          <td><b>@lang('Код'):</b></td>
          <td>{{$user->student_code}}</td>
          <td><b>@lang('Сессия'):</b></td>
          <td>{{$user->studentInfo['session']}}</td>
          @else
          <td><b>@lang('Код'):</b></td>
          <td>{{$user->student_code}}</td>
          <td><b>@lang('О себе'):</b></td>
          <td>{{$user->about}}</td>
          @endif
        </tr>
        @if($user->role == "student")
        <tr>
          <td><b>@lang('Класс'):</b></td>
          <td>{{$user->section->class->class_number}}</td>
          <td><b>@lang('Раздел'):</b></td>
          <td>{{$user->section->section_number}}</td>
        </tr>
        <tr>
          <td><b>@lang('Версия'):</b></td>
          <td>{{$user->studentInfo['version']}}</td>
          <td><b>@lang('Группа крови'):</b></td>
          <td>{{$user->blood_group}}</td>
        </tr>
        <tr>
          <td><b>@lang('Групп'):</b></td>
          <td>{{$user->studentInfo['group']}}</td>
          <td><b>@lang('День рождение'):</b></td>
          <td>{{Carbon\Carbon::parse($user->birthday)->format('d/m/Y')}}</td>
        </tr>
        @endif
        <tr>
          <td><b>@lang('Нация'):</b></td>
          <td>{{$user->nationality}}</td>
          <td><b>@lang('Религия'):</b></td>
          <td>{{$user->studentInfo['religion']}}</td>
        </tr>
        @if($user->role == "student")
        <tr>
          <td><b>@lang('Имя отца'):</b></td>
          <td>{{$user->studentInfo['father_name']}}</td>
          <td><b>@lang('Имя мамы'):</b></td>
          <td>{{$user->studentInfo['mother_name']}}</td>
        </tr>
        @endif
        <tr>
          <td><b>@lang('Адрес'):</b></td>
          <td>{{$user->address}}</td>
          <td><b>@lang('Номер телефона'):</b></td>
          <td>{{$user->phone_number}}</td>
        </tr>
        @if($user->role == "student")
        <tr>
