            ...
            <div class="panel panel-default">
              @if(count($grades) > 0)
                ...
                @foreach ($grades as $grade)
                    <h4>
                        @lang('Оценку поставил')
                        @if(count($grades) > 0)
                            @foreach ($grades as $grade)
                                <b>@lang('Код учителя')</b> - {{$grade->teacher->student_code}} &nbsp;<b>@lang('Имя')</b> - {{$grade->teacher->name}}
                                @break($loop->first)
                            @endforeach
                        @else
                            @lang('Нету данных.')
                    ...
                    <div class="row" style="margin-bottom: 2%;">
                      <div class="col-md-10">
                        <b>@lang('Курс') - </b> <small>{{$grade->course->course_name}} | </small>
                        <b> @lang('Класс') - </b> <small>{{$grade->course->section->class->class_number}} | </small>
                          <b> @lang('Раздел') - </b> <small>{{$grade->course->section->section_number}} | </small>
                        <b> @lang('Экзамен') - </b> <small>{{$grade->exam->exam_name}}</small>
                      </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="col-md-4">
                      <div class="alert alert-info" role="alert">
                        <ul>
                          <li>@lang('Куиз считается лучшим') <span class="label label-success">{{$grade->course->quiz_count}}</span></li>
                        </ul>
                        </div>
                      </div>
                    @endif
                    @if($grade->course->assignment_count > 0)
                      <div class="col-md-4">
                        <div class="alert alert-info" role="alert">
                        <ul>
                          <li>@lang('Упражнение считается лучшим ')<span class="label label-success">{{$grade->course->assignment_count}}</span></li>
                        </ul>
                        </div>
                      </div>
                    @endif
                    @if($grade->course->ct_count > 0)
                      <div class="col-md-4">
                        <div class="alert alert-info" role="alert">
                        <ul>
                          <li>@lang('Клас считается лучшим') <span class="label label-success">{{$grade->course->ct_count}}</span></li>
                        </ul>
                        </div>
                      </div>
                    @endif
